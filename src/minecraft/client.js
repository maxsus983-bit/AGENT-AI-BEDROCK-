...
function emit(event, payload = {}) {
    ...
    try {
        if (typeof context.eventBus.emitSafe === 'function') {
            context.eventBus.emitSafe(event, payload);
        } else if (typeof context.eventBus.emit === 'function') {
...
function updatePositionFromPacket(packet) {
    ...
    if (position) {
        ...
        emit('minecraft:position', {
            position,
            packet: packetName(packet)
        });
    }

    const rotation = extractRotation(packet);

    if (rotation) {
        context.state.rotation = rotation;

        emit('minecraft:rotation', {
            rotation,
            packet: packetName(packet)
        });
    }
...
function handlePacket(packet) {
    ...
    const name = packetName(packet);

    emit('minecraft:packet', {
        name,
        packet
    });
    ...
    if (observationPackets.has(name)) {
        emit('minecraft:observation_packet', {
            name,
            packet
        });
    }
    ...
    if (combatPackets.has(name)) {
        emit('minecraft:combat_packet', {
            name,
            packet
        });
    }
    ...
    if (worldPackets.has(name)) {
        emit('minecraft:world_packet', {
            name,
            packet
        });
    }
...
function installClientHandlers() {
    ...
    client.on('connect', () => {
        ...
        logger().success?.(
            'Minecraft serverga RakNet ulanish o‘rnatildi.'
        );

        emit('minecraft:connected', {
            host: context?.config?.minecraft?.host,
            port: context?.config?.minecraft?.port,
            timestamp: Date.now()
        });
    });
    ...
    client.on('join', packet => {
        ...
        logger().success?.(
            'Minecraft server join bosqichi muvaffaqiyatli.'
        );
    });
...
function createClient() {
    ...
    const mc = context.config.minecraft;
    ...
    bedrock-protocol odatda createClient() vaqtida
    serverga RakNet ping yuboradi.

    GitHub Actions / Replit / boshqa cloud runnerlarda
    aynan shu UDP ping timeout bo‘lishi mumkin.

    skipPing true bo‘lsa, dastlabki ping o'tkazib yuboriladi
    va client to‘g‘ridan-to‘g‘ri ulanishga harakat qiladi.
    ===========================================================
    */
    ...
    logger().info(
        `Bedrock options: host=${options.host}, port=${options.port}, version=${options.version}, username=${options.username}, offline=${options.offline}, skipPing=${options.skipPing}, timeout=${options.connectTimeout}`
    );
...
function sendPacket(name, params = {}) {
    ...
    try {
        /*
        bedrock-protocol write() orqali packet yuboradi.
        */
...
async function initialize(agentContext) {
    ...
    /*
    connect() shu yerda faqat bir marta chaqiriladi.
    index.js yana connect() qilsa ham client ulangan bo‘lsa
    ikkinchi ulanish yaratilmaydi.
    */
